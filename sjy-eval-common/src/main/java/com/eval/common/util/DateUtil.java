...
public class DateUtil {
	 ...
	 * @return
	 */
	public static Timestamp getNowInMillis(Long d) {
		Timestamp timeStamep = new Timestamp(getNow().getTime() + d);
		return timeStamep;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(minuteBetweenByDateStr(getDateByStrTime("201801052359", "yyyyMMddHHmm"),
				getDateByStrTime("201801060000", "yyyyMMddHHmm")));

		// 2.活动结束时间>0 （ 默认2018年1月5号 20:30结束）
		if (DateUtil.minuteBetweenByDateStr(DateUtil.getDateByStrTime("201801111348", "yyyyMMddHHmm"),
				DateUtil.getNow()) > 0) {
			System.out.println(DateUtil.minuteBetweenByDateStr(
					DateUtil.getDateByStrTime("201801111348", "yyyyMMddHHmm"), DateUtil.getNow()));
		} else {
